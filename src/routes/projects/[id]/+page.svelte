    ...
    {#if project}
      <div class="w-full p-8 pt-[100px] max-w-[900px] mx-auto">
        ...
        <div class="mb-8">
          ...
          <p class="text-xl text-[#7a8293] mb-2">{role}</p>
          <p class="text-lg text-[#7a8293] mb-4">{shortDescription}</p>
          <!-- {#each toolsUsed.split(',') as tool (tool)}
          <span class="text-[#000] text-[10px] bg-[#f6f8f7] px-2 py-[2px] rounded-full">{tool.trim()}</span>
          {/each} -->
          <p class="text-[#7a8293] mb-4"><strong>Tools:</strong> {toolsUsed}</p>
          {#if link}
        ...
        />
        <div class="mb-8">
          <h2 class="text-2xl font-semibold mb-1">Description</h2>
          <p class="text-[#7a8293] leading-relaxed font-thin">{longDescription}</p>
        </div>

        {#if (images.length > 0 && title !== 'Pet Health')}
          <div class="mb-8">
            <h2 class="text-2xl font-semibold mb-4">Project Images</h2>
            <div class="flex items-center flex-col justify-start gap-4">
              {#each images as image, index}
                <div class="bg-gray-100 shadow-2xl rounded-lg overflow-hidden">
